    ...
    <el-form ref="loginFormRef" :model="addCandidateForm" :rules="addCandidateFormRules">
      <el-row :gutter="20">
        <el-form-item class="el-form-item" prop="name">
          ...
          <el-col :span="18">
            <el-input v-model="addCandidateForm.name" disabled></el-input>
          </el-col>
        </el-form-item>
        ...
        <el-button type="primary" @click="updateCandidate">提交</el-button>
      </el-row>
    </el-form>
  </div>
</template>
...
export default {
  ...
  props: ["candidateId", "updatedialogClose"],
  methods: {
    updateCandidate() {
      if (
        ...
        updateCandidateById(this.candidateId, this.addCandidateForm)
          .then((res) => {
            if (res.err == 0) {
              this.$message("修改成功");
  ...
  created() {
    ...
    getCandidateById(this.candidateId).then((res) => {
      if (res.err == 0 && res.total >= 1) {
        this.addCandidateForm = res.data[0];
      }
